...
const EditBikeForm: React.FC = () => {
  ...
  const BikeData= useSelector((states: RootStateOrAny) => states.ReducerComponent);
  ...
  useEffect(() => {
    const bike: EditTypes = BikeData?.find((item: EditTypes) => item.id === id);
  ...
  const handleInputFileChange = (e: React.ChangeEvent<HTMLInputElement>): void => {
    if (e.target.files && e.target.files[0]) {
     const reader:RootStateOrAny = new FileReader();
      reader.onload = (): void => {
                        ...
                        <Box sx={{ display: 'flex', justifyContent: 'center' }}>
                          <Box className={classes.imageShow}>
                            ...
                            <label htmlFor="icon-button-file">
                              ...
                              <PhotoCamera
                                className={img ? `${classes.AfterPic}` : `${classes.camera}`}
                              />
                            </label>
                          </Box>
                        </Box>
