...
const EventPage: React.FC = () => {
  ...
  const [StoreEvent, dispatch] = useReducer<React.Reducer<RootStateOrAny,RootStateOrAny>>(ReducerTs, []);
  ...
  const BookNew = (state:RootStateOrAny): void => {
    (async () => {
      await axios
        .post('http://localhost:3004/Events', state)
        .then((res: RootStateOrAny) => {
