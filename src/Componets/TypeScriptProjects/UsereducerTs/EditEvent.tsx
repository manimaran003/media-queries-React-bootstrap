...
const EditEvent: React.FC = () => {
  ...
  const { id } = useParams();
  const [fetchData, setFetch] = useState<EditType>({
    username: '',
    eventName: '',
    members: '',
    time: ''
    ...
    (async () => {
      await axios
        ...
        .then((res: RootStateOrAny) => {
          ...
          const data = res.data.find((item: RootStateOrAny) => item.id === id);
          console.log(data);
          setFetch(data);
        })
        .catch((err: string) => {
  ...
  const handleSubmit = (e: React.MouseEvent<HTMLButtonElement>): void => {
    ...
    axios
      .put(`http://localhost:3004/Events/${id}`, state)
      .then((res: RootStateOrAny) => {
        console.log(res.data);
        navigate(`/typeEvent`);
      })
