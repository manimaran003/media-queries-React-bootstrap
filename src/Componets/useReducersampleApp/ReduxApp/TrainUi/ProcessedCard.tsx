...
export default function ProcessedCard() {
  ...
  const data= useSelector((state:RootStateOrAny) => state.PostReducer.userTrain);
  ...
  const handleEdit = (e: RootStateOrAny) => {
    e.preventDefault();
    setOpen(true);
  };
  const handleClose = () => {
  ...
  const handleDelete = (e: RootStateOrAny) => {
    e.preventDefault();
  ...
  return (
    <Card sx={{ maxWidth: 500, minHeight: 500 }}>
      ...
      <CardContent sx={{ mt: 5 }}>
        ...
        <Box>time: {train.time}</Box>
        <Box>arrival:{train.arrival}</Box>
