...
const useStyles = makeStyles({
  root: {
    marginTop: '50px',
    background:
      'linear-gradient(139deg, rgba(2,0,36,1) 0%, rgba(191,232,227,1) 0%, rgba(255,255,255,1) 99%, rgba(0,251,156,1) 100%);',
    height: '860px'
  },
  text: {
    paddingBottom: '20px',
...
const BookTrain: React.FC = () => {
  ...
  const data= useSelector((states: RootStateOrAny) => states.PostReducer.trips);
  const { id } = useParams();
  useEffect(() => {
    const valued = data ? data.find((item: RootStateOrAny) => item.id === id) : '';
    setValue(valued);
  }, [data, id]);
  type IUser = {
    passengerAge: string;
        ...
        <Box sx={{ p: 5 }}>
          <Grid container spacing={2}>
                  ...
                  <FormControl>
                    <Grid container spacing={2}>
                      <Grid item xs={12}>
                        ...
                        <Box className={classes.bookBox}>
                          <Button disabled={btn} onClick={handleSubmit}>
                            BookTrain
                          </Button>
                        </Box>
                      </Grid>
            ...
            <Grid item xs={12} sm={12} md={6} lg={6} xl={6}>
              ...
              <Box className={classes.bookBox}>{open && <ProcessedCard />}</Box>
            </Grid>
          </Grid>
        </Box>
      </Box>
