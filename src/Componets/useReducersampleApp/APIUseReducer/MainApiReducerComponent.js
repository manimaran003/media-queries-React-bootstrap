...
const MainApiReducerComponent = () => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3006/Trains')
      .then((res) => {
  ...
  const AddNewTrain = (data) => {
    (async () => {
      await axios
        .post(`http://localhost:3006/Trains`, data)
        .then((res) => {
          if (res.status === 201) toast('successfully added new train');
  ...
  const DeleteTrain = (id) => {
    (async () => {
      await axios
        .delete(`http://localhost:3006/Trains/${id}`)
        .then((res) => {
          if (res.status === 200) {
  ...
  const EditTrain = (id, data) => {
    (async () => {
      await axios
        .put(`http://localhost:3006/Trains/${id}`, data)
        .then((res) => {
          if (res.status === 200) {
